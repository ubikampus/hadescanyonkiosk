...
sudo cat > /etc/X11/Xwrapper.config << EOL
...
sudo cat > /home/kiosk/kioskx11.sh << EOL
#!/bin/bash
...
sudo cat > /home/kiosk/runkiosk.sh << EOL
...
sudo cat > /etc/systemd/system/kiosk.service << EOL
[Unit]
Description=Kiosk
...
sudo cat >> /etc/apt/apt.conf.d/50unattended-upgrades << EOL
...
sudo cat > /etc/apt/apt.conf.d/20auto-upgrades << EOL
